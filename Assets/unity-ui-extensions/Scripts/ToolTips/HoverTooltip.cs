...
namespace Assets.Scripts.ToolTips
{
    public class HoverTooltip : MonoBehaviour
    {
        ...
        public void OnScreenSpaceCamera()
        {
            ...
            var newPos = GUICamera.ScreenToViewportPoint(Input.mousePosition);
            ...
            if (val > upperRight.x)
            {
                var distFromRight = upperRight.x - val;
                ...
                var newTooltipPos = new Vector3(GUICamera.ViewportToScreenPoint(newPos).x + xOffSet, 0f, 0f);
            ...
            if (val < lowerLeft.x)
            {
                var distFromLeft = lowerLeft.x - val;
            ...
            val = GUICamera.ViewportToScreenPoint(newPos).y -
            ...
            if (val > upperRight.y)
            {
                var distFromUpper = upperRight.y - val;
                ...
                //assign the new modified coordinates to the tooltip and convert to screen coordinates
                var newTooltipPos = new Vector3(newPos.x, GUICamera.ViewportToScreenPoint(newPos).y + yOffSet, 0f);
            ...
            if (val < lowerLeft.y)
            {
                var distFromLower = lowerLeft.y - val;
                yOffSet = bgImage.sizeDelta.y*currentYScaleFactor*bgImage.pivot.y;
            ...
            transform.parent.transform.position = new Vector3(GUICamera.ViewportToWorldPoint(newPos).x,
                GUICamera.ViewportToWorldPoint(newPos).y, 0f);
