...
namespace Assets.Scripts.Effects
{
    ...
    public class ImageExtended : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter
    {
        ...
        [FormerlySerializedAs("m_Frame")] [SerializeField] private Sprite m_Sprite;
        ...
        public Sprite overrideSprite
        {
            ...
            set
            {
                if (SetPropertyUtility.SetClass(ref m_OverrideSprite, value)) SetAllDirty();
            }
        }
        ...
        public Rotate rotate
        {
            ...
            set
            {
                if (SetPropertyUtility.SetStruct(ref m_Rotate, value)) SetVerticesDirty();
            }
        }
        ...
        public override void SetNativeSize()
        {
            if (overrideSprite != null)
            {
                var w = overrideSprite.rect.width/pixelsPerUnit;
                var h = overrideSprite.rect.height/pixelsPerUnit;
        ...
        private void GenerateTiledSprite(List<UIVertex> vbo)
        {
            ...
            var rect = GetPixelAdjustedRect();
            var tileWidth = (spriteSize.x - border.x - border.z)/pixelsPerUnit;
            var tileHeight = (spriteSize.y - border.y - border.w)/pixelsPerUnit;
            border = GetAdjustedBorders(border/pixelsPerUnit, rect);
            ...
            var xMin = border[rx];
            var xMax = rect.width - border[rz];
            var yMin = border[ry];
            var yMax = rect.height - border[rw];
            ...
            if (m_FillCenter)
            {
                for (var y1 = yMin; y1 < yMax; y1 += tileHeight)
                {
                    ...
                    if (y2 > yMax)
                    {
                        ...
                        s_UVTiled[k1][k2] = s_UVTiled[1 - k1][k2] +
                                            (s_UVTiled[k1][k2] - s_UVTiled[1 - k1][k2])*(yMax - y1)/(y2 - y1);
                        y2 = yMax;
                    }
                    for (var x1 = xMin; x1 < xMax; x1 += tileWidth)
                    {
                        ...
                        if (x2 > xMax)
                        {
                            ...
                            s_UVTiled[k1][k2] = s_UVTiled[1 - k1][k2] +
                                                (s_UVTiled[k1][k2] - s_UVTiled[1 - k1][k2])*(xMax - x1)/(x2 - x1);
                            x2 = xMax;
                        }
                        AddQuad(vbo, v, new Vector2(x1, y1) + rect.position, new Vector2(x2, y2) + rect.position,
        ...
        private void AddQuad(List<UIVertex> vbo, UIVertex v, Vector2 posMin, Vector2 posMax, Vector2 uvMin,
            Vector2 uvMax)
        {
        ...
        private Vector4 GetAdjustedBorders(Vector4 border, Rect rect)
        {
            for (var axis = 0; axis <= 1; axis++)
            {
                ...
                var rectSize = rect.size[(axis + (int) rotate%2)%2];
                if (rectSize < combinedBorders && combinedBorders != 0)
                {
                    var borderScaleRatio = rectSize/combinedBorders;
        ...
        private void GenerateFilledSprite(List<UIVertex> vbo, bool preserveAspect)
        {
            ...
            var v = GetDrawingDimensions(preserveAspect);
            var outer = overrideSprite != null ? DataUtility.GetOuterUV(overrideSprite) : Vector4.zero;
            var uiv = UIVertex.simpleVert;
            uiv.color = color;
            ...
            if (m_FillMethod == FillMethod.Horizontal || m_FillMethod == FillMethod.Vertical)
            {
                ...
                {
                    var fill = (outer[rz] - outer[rx])*m_FillAmount;
                ...
                {
                    var fill = (outer[rw] - outer[ry])*m_FillAmount;
            ...
            if (m_FillAmount < 1f)
            {
                ...
                if (fillMethod == FillMethod.Radial180)
                {
                    for (var side = 0; side < 2; ++side)
                    {
                        ...
                        var val = m_FillClockwise ? fillAmount*2f - side : m_FillAmount*2f - (1 - side);
