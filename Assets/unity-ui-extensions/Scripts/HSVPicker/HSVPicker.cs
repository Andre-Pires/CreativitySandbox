...
namespace Assets.Scripts.HSVPicker
{
    public class HSVPicker : MonoBehaviour
    {
        ...
        private void Awake()
        {
            hsvSlider.texture = HSVUtil.GenerateHSVTexture((int) hsvSlider.rectTransform.rect.width,
                (int) hsvSlider.rectTransform.rect.height);
            ...
            hsvImage.texture = HSVUtil.GenerateColorTexture((int) hsvImage.rectTransform.rect.width,
                (int) hsvImage.rectTransform.rect.height, ((Texture2D) hsvSlider.texture).GetPixelBilinear(0, 0));
        ...
        public Color MoveCursor(float posX, float posY, bool updateInputs = true)
        {
            ...
            cursorX = posX;
            cursorY = posY;
            cursor.rectTransform.anchoredPosition = new Vector2(posX*hsvImage.rectTransform.rect.width,
                posY*hsvImage.rectTransform.rect.height - hsvImage.rectTransform.rect.height);
        ...
        {
            //Debug.Log(posX + " " + posY);
            return ((Texture2D) hsvImage.texture).GetPixel((int) (cursorX*hsvImage.texture.width),
                (int) (cursorY*hsvImage.texture.height));
        }
        ...
        public Color MovePointer(float newPos, bool updateInputs = true)
        {
            ...
            {
                if ((int) hsvImage.rectTransform.rect.width != hsvImage.texture.width ||
                    (int) hsvImage.rectTransform.rect.height != hsvImage.texture.height)
                {
                    Destroy(hsvImage.texture);
                    hsvImage.texture = null;

                    hsvImage.texture = HSVUtil.GenerateColorTexture((int) hsvImage.rectTransform.rect.width,
                        (int) hsvImage.rectTransform.rect.height, mainColor);
                }
                else
                {
