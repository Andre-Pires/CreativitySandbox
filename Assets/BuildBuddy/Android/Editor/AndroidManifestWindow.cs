...
namespace BuildBuddy
{
    public class AndroidManifestWindow : EditorWindow
    {
        ...
        {
            GetWindow<AndroidManifestWindow>(false, "AndroidManifest Editor");
        }
        ...
        private void OnGUI()
        {
            ...
            {
                var templateEditor = new AndroidXmlEditor(data.ToString());
                var newTemplate = AndroidWindowData.CreateInstance(templateEditor);
            ...
            for (var i = 0; i < templateElements.Count; i++)
            {
                EditorGUILayout.BeginHorizontal();
                {
                    templateElements[i].display = EditorGUILayout.Foldout(templateElements[i].display,
