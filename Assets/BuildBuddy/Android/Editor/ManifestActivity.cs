...
namespace BuildBuddy
{
    ...
    public sealed class ManifestActivity : ManifestElement
    {
        ...
        [SerializeField] private readonly List<ManifestIntentFilter> intentFilterList = new List<ManifestIntentFilter>();
        [SerializeField] private string label = "";
        [SerializeField] private string launchMode = launchModeArray[0];
        [SerializeField] private readonly List<ManifestMetaData> metaDataList = new List<ManifestMetaData>();
        ...
        [SerializeField] private int windowSoftInputMode;

        //Constructed by editor window
        public static ManifestActivity CreateInstance()
        {
            var activity = CreateInstance<ManifestActivity>();
            activity.node = null;
            ...
            if (display)
            {
                ...
                if (displayAttributes)
                {
                    ...
                    BBGuiHelper.BeginIndent();
                    {
                        ...
                        alwaysRetainTaskState = EditorGUILayout.Toggle("Always Retain Task State: ",
                        ...
                        launchMode =
                            launchModeArray[
                                EditorGUILayout.Popup("Launch Mode: ", Array.IndexOf(launchModeArray, launchMode),
                                    launchModeArray)];
                        ...
                        screenOrientation =
                            screenOrientationArray[
                                EditorGUILayout.Popup("Screen Orientation: ",
                                    Array.IndexOf(screenOrientationArray, screenOrientation), screenOrientationArray)];
                        ...
                        uiOptions =
                            uiOptionsArray[
                                EditorGUILayout.Popup("UI Options: ", Array.IndexOf(uiOptionsArray, uiOptions),
                                    uiOptionsArray)];
                        windowSoftInputMode = EditorGUILayout.MaskField("Window Soft Input Mode: ", windowSoftInputMode,
                            windowSoftInputModeArray);
                    }
                ...
                displayIntentFilter = EditorGUILayout.Foldout(displayIntentFilter,
                    "Intent-Filters: (" + intentFilterList.Count + ")");
                if (displayIntentFilter)
                {
                    BBGuiHelper.BeginIndent();
                    {
                        for (var i = 0; i < intentFilterList.Count; i++)
                        {
        ...
        private void InitializeConfigChanges()
        {
            ...
            var changes = node.Attributes["android:configChanges"].Value.Split('|');
        ...
        {
            var configChangesString = "";
            for (var i = 0; i < configChangesArray.Length; i++)
            {
        ...
        private void InitializeWindowSoftInputMode()
        {
            ...
            var modes = node.Attributes["android:windowSoftInputMode"].Value.Split('|');
        ...
        {
            var modeString = "";
            for (var i = 0; i < windowSoftInputModeArray.Length; i++)
            {
        ...
        protected override void CreateNode(XmlDocument document)
        {
            ...
            var applicationElement = document.GetElementsByTagName("application")[0];
        ...
        protected override void UpdateAttributes(XmlDocument document)
        {
            ...
            for (var i = 0; i < intentFilterList.Count; i++)
            {
                var intentFIlter = intentFilterList[i];
            ...
            UpdateOptionalAttribute(document, "screenOrientation", screenOrientation != screenOrientationArray[0],
                screenOrientation);
            ...
            UpdateOptionalAttribute(document, "windowSoftInputMode", windowSoftInputMode != 0,
                WindowSoftInputModeToString());
        }
