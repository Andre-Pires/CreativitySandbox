...
namespace BuildBuddy
{
    ...
    public sealed class ManifestProvider : ManifestElement
    {
        ...
        [SerializeField] private readonly List<ManifestGrantUriPermission> grantUriPermissionList =
            new List<ManifestGrantUriPermission>();
        ...
        [SerializeField] private readonly List<ManifestPathPermission> pathPermissionList =
            new List<ManifestPathPermission>();
        ...
        {
            var provider = CreateInstance<ManifestProvider>();
            provider.node = null;
        ...
        public override void OnGUI()
        {
            ...
            if (display)
            {
                ...
                BBGuiHelper.BeginIndent();
                {
                    ...
                    displayGrantUriPermissions = EditorGUILayout.Foldout(displayGrantUriPermissions,
                        "Grant-Uri-Permissions: (" + grantUriPermissionList.Count + ")");
                    if (displayGrantUriPermissions)
                    {
                        for (var i = 0; i < grantUriPermissionList.Count; i++)
                        {
                    ...
                    displayPathPermissions = EditorGUILayout.Foldout(displayPathPermissions,
                        "Path-Permissions: (" + pathPermissionList.Count + ")");
                    if (displayPathPermissions)
                    {
                        for (var i = 0; i < pathPermissionList.Count; i++)
                        {
        ...
        private void Initialize()
        {
            ...
            {
                initOrder = Convert.ToInt32(node.Attributes["android:icon"].Value);
            }
        ...
        protected override void UpdateAttributes(XmlDocument document)
        {
            ...
            for (var i = 0; i < grantUriPermissionList.Count; i++)
            {
                var uriPermission = grantUriPermissionList[i];
            ...
            for (var i = 0; i < pathPermissionList.Count; i++)
            {
                var pathPermission = pathPermissionList[i];
