...
namespace Assets.Scripts.Classes.Agent.ComposedBehaviors
{
    ...
    {
        private readonly Color _behaviorColor = Configuration.Instance.ColorNames[Configuration.Colors.Blue];
                ...
                foreach (Behavior behavior in ExcitedBehaviors)
                {
                    switch (behavior.BehaviorType)
                    {
                        ...
                        case Configuration.Behaviors.Rotate:
                            (behavior as RotationBehavior).PrepareBehavior(body, 180.0f, Configuration.RotationDirection.Right, 
                                Configuration.Transitions.EaseIn, 1, duration);
                            break;
                        default:
                    ...
                    switch (behavior.BehaviorType)
                    {
                        ...
                        case Configuration.Behaviors.Rotate:
                            (behavior as RotationBehavior).PrepareBehavior(body, 180.0f, Configuration.RotationDirection.Alternating, 
                                Configuration.Transitions.EaseOut, 1, 7);
                            break;
                        default:
                            throw new ArgumentOutOfRangeException();
                    }
