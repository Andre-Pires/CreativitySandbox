...
namespace UnityStandardAssets.Cameras
{
    public class TargetFieldOfView : AbstractTargetFollower
    {
        ...
        [SerializeField] private readonly float m_FovAdjustTime = 1;
            // the time taken to adjust the current FOV to the desired target FOV amount.

        private float m_FovAdjustVelocity;

        [SerializeField] private readonly bool m_IncludeEffectsInSize = false;
            // changing this only takes effect on startup, or when new target is assigned.

        private Transform m_LastTarget;

        [SerializeField] private readonly float m_ZoomAmountMultiplier = 2;
        ...
        protected override void FollowTarget(float deltaTime)
        {
            ...
            var dist = (m_Target.position - transform.position).magnitude;
            var requiredFOV = Mathf.Atan2(m_BoundSize, dist)*Mathf.Rad2Deg*m_ZoomAmountMultiplier;

            m_Cam.fieldOfView = Mathf.SmoothDamp(m_Cam.fieldOfView, requiredFOV, ref m_FovAdjustVelocity,
                m_FovAdjustTime);
        }
