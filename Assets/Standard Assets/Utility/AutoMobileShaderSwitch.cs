        ...
        private void OnEnable()
        {
            ...
            foreach (var replacementDef in m_ReplacementList.items)
            {
                foreach (var r in renderers)
                {
                    ...
                    for (var n = 0; n < r.sharedMaterials.Length; ++n)
                    {
                        ...
                        if (material.shader == replacementDef.original)
                        {
                            ...
                            Debug.Log("replacing " + r.gameObject.name + " renderer " + n + " with " +
            ...
            for (var n = 0; n < oldMaterials.Count; ++n)
            {
                Debug.Log(oldMaterials[n].name + " (" + oldMaterials[n].shader.name + ")" + " replaced with " +
                          newMaterials[n].name + " (" + newMaterials[n].shader.name + ")");
            }
#endif
        }
